...
RStudio (Posit) maintains a collection of wonderful [cheatsheets](https://rstudio.github.io/cheatsheets/). The following will be helpful:
...
When you want to check for a set containment, the `%in%` operator is the correct way to do this, `==` is not.
...
```{r is_na}
x <- c(1, 4, 9, NA)
x == NA # is x equal to "i don't know?" well we don't know
is.na(x)
```
...
Note: `ifelse()` (from base R) and `if_else()` (from tidyverse) are different functions. We prefer `if_else()` for many reasons:

- It is noisy to make sure we catch issues/bugs
- It can explicitly handle missing values
- It keeps dates as dates

Here are few examples comparing `ifelse()` and `if_else()`:
...
#3
diamonds |> 
  mutate(is_fpi = cut %in% c("Fair", "Premium", "Ideal")) |> # creates new column TRUE/FALSE of if the diamond is one of those cuts or not
  summarize(num_fpi = sum(is_fpi), # works because TRUE = 1 and FALSE = 0, so it's a tally of all the TRUEs
            frac_fpi = mean(is_fpi)) # works for same reason as above; takes fraction of 1s (TRUEs) divided by total number of observations
...
diamonds |> 
  ...
  mutate(price_cat2 = case_when(price < 500 ~ "low",
...
lakers_clean |> select(year:second) |> head()
...
lakers_clean |> select(date, opponent, time, period, diff_btw_plays_sec) |> head()
```
...
What if we try to create a factor with values that aren't in the levels, eg, a typo in a month name?
